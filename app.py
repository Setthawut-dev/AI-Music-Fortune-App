...
if st.button("🌟 วิเคราะห์ดวงชะตาอย่างละเอียด"):
    if artist and song:
        with st.spinner('🔭 กำลังเปิดคัมภีร์ดนตรีทำนายดวง...'):
            ...
            # 💡 เพิ่มรุ่นใหม่ล่าสุดเข้าไปในลิสต์ (gemini-2.0-flash และ 2.5)
            fallback_models = [
                "gemini-2.5-flash",
                "gemini-2.0-flash",
                "gemini-1.5-flash",
            ...
            success = False
            error_logs = [] # 🕵️‍♂️ กล่องเก็บความลับของ Error
            ...
            for m_name in fallback_models:
                try:
                    ...
                    # 🕵️‍♂️ ถ้า Error ให้เก็บข้อความนั้นไว้
                    error_logs.append(f"**{m_name}** ➡️ `{str(e)}`")
                    continue 
            
            if not success:
                st.error("🚨 ไม่สามารถทำนายได้! นี่คือสาเหตุที่แท้จริงจาก Google:")
                for err in error_logs:
                    st.warning(err)
                st.info("💡 รบกวนก๊อปปี้ข้อความ Error สีเหลืองด้านบนมาให้ผมดูหน่อยนะครับ เราจะได้แก้ได้ตรงจุด!")

    else:
        st.warning("⚠️ กรุณากรอกชื่อศิลปินและเพลงก่อนนะครับ")

st.write("---")
st.caption("Music Fortune v12.0 | Debug Mode")
